...
## Refinement pass: scroll section indicator

`DayView.vue` gained a fixed 3-dot indicator (right edge, vertically centered) showing which of the 3 sections (Welcome/Schedule/Registration) is currently in view — the active dot is filled `--color-brand-green`, the others stay hollow. Active-section detection uses an `IntersectionObserver` scoped to the `.day-view` scroll container (50% visibility threshold) rather than Bootstrap's Scrollspy, since Scrollspy needs its JS bundle (only `bootstrap.min.css` is imported here) and expects nav-link/id markup that doesn't fit this single-page scroll-snap layout. Clicking a dot also smooth-scrolls to that section.
