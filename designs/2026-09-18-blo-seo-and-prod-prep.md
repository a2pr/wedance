...
- Hero flyer (`blo.jpeg`) bakes in a lot of small text (13-line prize list, Pix email/phone, dates) that becomes hard to read once scaled to a ~340px mobile screen — the image was already rendering at its max allowed width (`min(100%, 560px)`), so it wasn't being artificially shrunk further. Rather than commission a redesigned mobile-only graphic or crop the flyer (losing information not otherwise duplicated as real text, like the Pix email/phone), added a tap-to-zoom lightbox: tapping/clicking the flyer opens it at native 1024×1024 resolution inside a scrollable overlay (mirrors `ConfirmModal.vue`'s Bootstrap modal classes and Escape-to-close pattern), giving roughly 3x the effective reading size on mobile plus native pinch-zoom. Works on both mobile and desktop.
- Hit a CSS shrink-to-fit quirk while building the zoom trigger: the `<button>` wrapping the image is a flex item (child of `.container.d-flex`) with a percentage-width (`min(100%, 560px)`) image inside — browsers can't resolve that percentage during the button's own intrinsic-size calculation, so they fall back to the image's *intrinsic* 1024px width for sizing the button, while the image itself renders at the resolved 560px. Net effect: the button's box was 1024px wide with the 560px image left-aligned inside it, so the "toque para ampliar" badge (anchored to the button's bottom-right corner) floated ~460px away from the visible image. Fixed by moving the explicit width (`min(100%, 560px)`) onto the button itself and having the image just fill it at `width: 100%`, removing the percentage-resolution ambiguity entirely.
...
- `src/components/LotteryHeroSection.vue` / `src/constants/lotteryUi.ts` — flyer image is now a button that opens a full-resolution, scrollable lightbox on tap/click (Bootstrap `.modal`/`.modal-backdrop` classes, Escape-to-close via the same `watch` + `addEventListener('keydown', ...)` pattern as `ConfirmModal.vue`). New labels: `LOTTERY_FLYER_ZOOM_HINT`, `LOTTERY_FLYER_CLOSE_LABEL`.
